...
def find_top_grad(
    ...
    #     device=device,
    # )
    preds = torch.zeros(len(train_dataset), device=device)
    targets = torch.zeros(len(train_dataset), dtype=torch.long, device=device)
    loader = DataLoader(
        train_dataset,
        8192 * 4,
        pin_memory=True,
    ...
    for data, target, index in loader:
        ...
        preds[index] = y_pred
        # preds[index, 0] = 1 - y_pred
        # preds[index, 1] = y_pred

        targets[index] = target

    end_infer = time.time()
    print("Infer time", end_infer - start_infer)
    ...
    start = 0
    for _ in tqdm.tqdm(range(batches), ascii=True):

        end = start + random_batch_size
        tmp_index = generate_random_set(
            random_batch_size, equal_num, neg_location, pos_location
        )
        tmp_index = tmp_index.to(device)

        tmp_pred = preds[tmp_index]
        tmp_pred.requires_grad_(True)
        loss = criterion_selflc(tmp_pred, targets[tmp_index], total_step + 1)
...
k = int(data_size * len(train_dataset) / 0.9)
# k = int(data_size * len(train_dataset))
if args.debug:
    batches = 33
