...
class CriteoDataset(Dataset):
    ...
    def __init__(
        ...
        if dataset_name == "debug":
            self._line_in_dataset = list(range(2048 * 10))
        else:
    ...
    def get_target(self):
        ...
        batch_size = 2048
        sampler = torch.utils.data.BatchSampler(self, batch_size, False)

        start = 0
        for batch in sampler:
            item, target = torch.utils.data.default_collate(batch)
            end = start + len(item)
...
class CriteoCL(CriteoDataset):
    def __init__(
        ...
        # hook my dataset
        if dataset_name == "debug":
            line_in_dataset = list(range(2048 * 10))
        else:
            line_in_dataset = list(torch.load(train_test_info)[dataset_name])
        line_in_dataset.sort()

        self.start = line_in_dataset[0]
        self.length = len(line_in_dataset)
    ...
    def __getitems__(self, indices: List[int]):
        indices = [self.start + idx for idx in indices]
        with self.env.begin(write=False) as txn:
            cursor = txn.cursor()
            keys = [struct.pack(">I", index) for index in indices]
            res = cursor.getmulti(keys)
