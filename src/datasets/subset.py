...
class SubsetGenerator:
    def __init__(self, greedy, smtk, equal_num=True):
        self.mode = self._get_mode(greedy)
        self.smtk = smtk
        self.equal_num = equal_num
    ...
    def generate_subset(
        ...
        fl_labels = targets[idx] - targets[idx].min()
        ...
        if len(fl_labels) > 50000:
            ...
            if lib_use == "submodlib":
                ...
                (
                    ...
                    ordering_time,
                    similarity_time,
                ) = facility_location_torch.get_orders_and_weights(
                    B,
                    preds,
                    "euclidean",
                    y=fl_labels,
                    # equal_num=True,
                    equal_num=self.equal_num,
                    mode=mode,
                    num_n=num_n,
                )
            else:
